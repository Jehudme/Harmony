...
HARMONY_REGISTER_COMPONENT_WITH_BASE(Harmony::Components::Renderable, Harmony::Components::Plane, plane);

namespace Harmony::Components {
	Plane::Plane(HARMONY_COMPONENTS_CONSTRUCTOR_ARGUMENTS) :
		Renderable(scene, entityId),
		size{100.0f, 100.0f},
		color{255, 0, 0, 255},
		frameColor{128, 0, 0, 255}
	{
		HARMONY_DEBUG("Initializing Plane component for entity {}", static_cast<uint32_t>(entityId));
		...
		try {
			...
			HARMONY_ASSERT(sx > 0.0f, "Plane size X must be positive");
			HARMONY_ASSERT(sz > 0.0f, "Plane size Z must be positive");
			...
			throw Harmony::Exceptions::ComponentInitializationException("Plane", e.what());
		}
	}
	
	Plane::~Plane() 
	{
		HARMONY_TRACE("Plane component destroyed for entity {}", static_cast<uint32_t>(entityId_));
	}

	void Plane::onRender()
	{
		try {
			DrawPlane(Vector3{ 0.0f, 0.0f, 0.0f }, size, color);
		}
		catch (const std::exception& e) {
			HARMONY_ERROR("Exception during Plane render for entity {}: {}", 
