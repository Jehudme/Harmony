...
namespace Harmony::Components
{
		...
		: Shape<sf::CircleShape>(configuration)
	{
		if (std::optional<float> radius = configuration.get<float>({ "radius" })) 
			...
			HARMONY_WARN("Circle component missing radius configuration"); 
			this->setRadius(45.0f);
		}

		if (std::optional<std::size_t> pointCount = configuration.get<std::size_t>({ "point_count" })) 
			this->setPointCount(pointCount.value());
		else {
			HARMONY_WARN("Circle component missing point count configuration"); 
