...
HARMONY_REGISTER_COMPONENT_FULL(sf::Drawable, Harmony::Components::Circle, Circle)
...
namespace Harmony::Components
{
	...
	{
		if (std::optional<float> radius = configuration.get<float>({ "radius" }); radius.has_value())
			this->setRadius(radius.value());
		else HARMONY_WARN("Circle component missing radius configuration"); this->setRadius(45.0f);

		if (std::optional<std::size_t> pointCount = configuration.get<std::size_t>({ "point_count" }); pointCount.has_value())
			this->setPointCount(pointCount.value());
