...
namespace Harmony
{
        ...
        : Object(configuration), isValid_(false)
    {
        ...
        if (const auto texturePathData = configuration->get({ "Path" }))
        {
        ...
        else if (const auto textureColorData = configuration->get({ "Color" }))
        {
            const auto colorConfig = create<Configuration>(textureColorData.value());
            sf::Color color = {
                static_cast<sf::Uint8>(colorConfig->get<int>({ "R" }).value_or(255)),
                static_cast<sf::Uint8>(colorConfig->get<int>({ "G" }).value_or(255)),
                static_cast<sf::Uint8>(colorConfig->get<int>({ "B" }).value_or(255)),
                static_cast<sf::Uint8>(colorConfig->get<int>({ "A" }).value_or(255))
            };

            sf::Vector2u size = {
                static_cast<unsigned int>(colorConfig->get<int>({ "Width" }).value_or(128)),
                static_cast<unsigned int>(colorConfig->get<int>({ "Height" }).value_or(128))
            };
        ...
        if (const auto repeatedData = configuration->get({ "Repeated" }))
        {
