...
namespace Harmony::Components {
	...
	{
		HARMONY_DEBUG("Initializing Cube component for entity {}", static_cast<uint32_t>(entityId));
		...
		try {
			...
			HARMONY_ASSERT(sx > 0.0f, "Cube size X must be positive");
			HARMONY_ASSERT(sy > 0.0f, "Cube size Y must be positive");
			HARMONY_ASSERT(sz > 0.0f, "Cube size Z must be positive");
			...
			HARMONY_TRACE("Cube component initialized - Size: ({}, {}, {})", size.x, size.y, size.z);
		}
		catch (const std::exception& e) {
			HARMONY_ERROR("Failed to initialize Cube component: {}", e.what());
			throw Harmony::Exceptions::ComponentInitializationException("Cube", e.what());
		}
	}
	
	Cube::~Cube() 
	{
		HARMONY_TRACE("Cube component destroyed for entity {}", static_cast<uint32_t>(entityId_));
	}
	...
	void Cube::onRender()
	{
		try {
			...
			HARMONY_ERROR("Exception during Cube render for entity {}: {}", 
